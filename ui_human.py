...
class UI_HumanPlayer(QtWidgets.QMainWindow):
    def __init__(self, player_num=None, memory_num=None, strategy_num=None, iter_num=None, parent=None):
        ...
        self.new_game = Game(player_num=self.player_num, memory_num=self.memory_num, strategy_num=self.strategy_num,
                             iter_num=self.iter_num, is_human=True)
        self.total = 0
        self.setupUi(self)
        self.show()
    ...
    def bar_button_click(self):
        self.total += 1
        if self.total > self.iter_num:
            return
        self.new_game.start_game(self.total - 1, 1)
        temp = ''
        temp += str(self.total) + '\t\n\n'
        barstate = 0
    ...
    def setupUi(self, MainWindow):
        ...
        self.label_img1.setGeometry(QtCore.QRect(20, self.player_num * 40 + 60, 500, 500))
        self.label_img1.setObjectName("label_img1")

        self.label_img2 = QtWidgets.QLabel(self.centralwidget)
        self.label_img2.setGeometry(QtCore.QRect(525, self.player_num * 40 + 60, 500, 500))
        self.label_img1.setObjectName("label_img2")

        self.view_short = QtWidgets.QPushButton('View Short Memory', self.centralwidget)
        self.view_short.setGeometry(QtCore.QRect(240, self.player_num * 40 + 35, 130, 20))
        ...
        self.view_long.setGeometry(QtCore.QRect(380, self.player_num * 40 + 35, 130, 20))
        self.view_long.setObjectName("view_long")
        self.view_long.clicked.connect(self.view_long_button_click)

        self.bar = QtWidgets.QPushButton('bar', self.centralwidget)
        self.bar.setGeometry(QtCore.QRect(20, self.player_num * 40 + 35, 100, 20))
        self.bar.setObjectName("bar")
        self.bar.clicked.connect(self.bar_button_click)

        self.home = QtWidgets.QPushButton('home', self.centralwidget)
